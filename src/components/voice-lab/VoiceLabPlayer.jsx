...
export default function VoiceLabPlayer({ challenge, onBack }) {
  ...
  const [status, setStatus] = useState('listen'); // listen, recording, compare, finished
  ...
  useEffect(() => {
    ...
    if (originalAudioRef.current && currentIndex < lines.length) {
      ...
      originalAudioRef.current.play().catch(() => {}); // catch auto-play restrictions
    }
  }, [currentIndex]);
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px', minHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 30 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            <span style={{ color: 'var(--muted)' }}>{currentIndex + 1} de {lines.length}</span>
          </div>
          <div style={{ height: 6, background: 'rgba(255,255,255,0.1)', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ width: `${((currentIndex) / lines.length) * 100}%`, height: '100%', background: 'var(--purple)', transition: '0.3s' }}></div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 30 }}>
        ...
        {/* Contexto */}
        <div style={{ background: 'var(--paper)', padding: 25, borderRadius: 20, border: '1px solid var(--line)', textAlign: 'center' }}>
          <p style={{ color: 'var(--muted)', fontSize: '1.1rem', fontStyle: 'italic', margin: 0 }}>
            "{currentLine.translation}"
          </p>
        </div>

        {/* English Text (Hidden during recording) */}
        <div style={{ 
          background: 'var(--bg)', padding: 30, borderRadius: 20, border: '2px dashed var(--line)', textAlign: 'center',
          transition: 'all 0.3s',
          filter: status === 'recording' ? 'blur(10px)' : 'none',
          opacity: status === 'recording' ? 0.3 : 1
        }}>
          {status === 'listen' || status === 'compare' ? (
            <h2 style={{ fontSize: '2rem', color: '#fff', margin: 0, fontFamily: '"Playfair Display", serif' }}>
              {currentLine.originalText}
            </h2>
          ) : (
            <h2 style={{ fontSize: '2rem', color: 'var(--muted)', margin: 0, fontFamily: '"Playfair Display", serif' }}>
              Escute e memorize...
            </h2>
          )}
        </div>
        ...
        {/* Action Area */}
        <div style={{ background: 'var(--paper)', padding: 30, borderRadius: 20, border: '1px solid var(--line)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
          ...
          {currentLine.audioUrl?.includes('.mp4') ? (
            <video 
              ...
              style={{ width: '100%', maxHeight: 350, borderRadius: 16, background: '#000', marginBottom: 10, display: status === 'recording' ? 'none' : 'block' }}
              controls={false}
            />
          ) : (
            <audio 
          ...
          {status === 'listen' && (
            <>
              <div style={{ display: 'flex', gap: 15, alignItems: 'center' }}>
                ...
                >
                  {isPlayingOrig ? '🔊 Ouvindo...' : '▶️ Ouvir Modelo'}
                </button>
                <button 
              ...
              <div style={{ width: '100%', height: 1, background: 'var(--line)', margin: '10px 0' }}></div>
          ...
          {status === 'compare' && (
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 30 }}>
              ...
              >
                🔁 Reprodução Alternada (Original → Sua Voz)
              </button>

              <div style={{ borderTop: '1px solid var(--line)', paddingTop: 20 }}>
                <h3 style={{ textAlign: 'center', color: '#fff', margin: '0 0 20px' }}>Como você avalia sua repetição?</h3>
                <div style={{ display: 'flex', gap: 15, justifyContent: 'center' }}>
                  <button onClick={() => handleAssessment('red')} style={{ flex: 1, padding: '15px', background: 'rgba(255, 71, 87, 0.1)', border: '1px solid #ff4757', color: '#ff4757', borderRadius: 12, fontWeight: 'bold', cursor: 'pointer' }}>
                    🔴 Preciso Praticar
                  </button>
                  <button onClick={() => handleAssessment('yellow')} style={{ flex: 1, padding: '15px', background: 'rgba(255, 165, 2, 0.1)', border: '1px solid #ffa502', color: '#ffa502', borderRadius: 12, fontWeight: 'bold', cursor: 'pointer' }}>
                    🟡 Quase Lá
                  </button>
                  <button onClick={() => handleAssessment('green')} style={{ flex: 1, padding: '15px', background: 'rgba(46, 213, 115, 0.1)', border: '1px solid #2ed573', color: '#2ed573', borderRadius: 12, fontWeight: 'bold', cursor: 'pointer' }}>
                    🟢 Mandou Bem!
                  </button>
                </div>
              </div>
